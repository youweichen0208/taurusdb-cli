...
import { printFlavorTable, printInstanceTable, validatePassword } from "../index.js";
...
describe("table rendering", () => {
  it("InstanceTable contains key fields", () => {
    ...
    const out = getOutput();
    for (const needle of ["实例列表", "prod-main-db", "creating-db", "gaussdb-mysql 8.0", "normal", "creating"]) {
  ...
  it("FlavorTable contains key fields", () => {
    printFlavorTable([
      { spec_code: "gaussdb.mysql.large.x86.2", vcpus: "2", ram: "8", type: "x86" },
      { spec_code: "gaussdb.mysql.xlarge.x86.4", vcpus: "4", ram: "16", type: "x86" }
    ]);
    const out = getOutput();
    for (const needle of ["规格列表", "gaussdb.mysql.large.x86.2", "gaussdb.mysql.xlarge.x86.4", "vCPU", "内存"]) {
      expect(out).toContain(needle);
    }
  });
});
